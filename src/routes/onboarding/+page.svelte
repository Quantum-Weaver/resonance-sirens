<script lang="ts">
	...
	import { SIRENS_THEMES } from '$lib/stores/theme.svelte';
	...
	const STEPS: StepDef[] = [
		...
		{ id: 'theme', kind: 'choose', atMost: 1, preset: ['rose'], offers: themeOffers }
	];
	...
	function finish() {
		...
		if (name) localStorage.setItem('resonance-sirens-vessel-name', name);
		themeStore.setPreset((done.choices['theme'] ?? ['rose'])[0] ?? 'rose');
